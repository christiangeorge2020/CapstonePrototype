...
bool PluginCore::initPluginParameters()
{
	...
	// --- continuous control: Input Rate
	piParam = new PluginParameter(controlID::rateAdjust_manual, "Input Rate", "", controlVariableType::kDouble, 0.500000, 2.000000, 1.000000, taper::kAntiLogTaper);
	piParam->setParameterSmoothing(false);
	piParam->setSmoothingTimeMsec(100.00);
	piParam->setBoundVariable(&rateAdjust_manual, boundVariableType::kDouble);
	addPluginParameter(piParam);

	// --- continuous control: Input Tempo
	piParam = new PluginParameter(controlID::tempoAdjust_manual, "Input Tempo", "", controlVariableType::kDouble, 0.500000, 2.000000, 1.000000, taper::kAntiLogTaper);
	piParam->setParameterSmoothing(false);
	piParam->setSmoothingTimeMsec(100.00);
	piParam->setBoundVariable(&tempoAdjust_manual, boundVariableType::kDouble);
	addPluginParameter(piParam);
	...
	// --- RAFX GUI attributes
	// --- controlID::rateAdjust_manual
	auxAttribute.reset(auxGUIIdentifier::guiControlData);
	auxAttribute.setUintAttribute(2147483648);
	setParamAuxAttribute(controlID::rateAdjust_manual, auxAttribute);

	// --- controlID::tempoAdjust_manual
	auxAttribute.reset(auxGUIIdentifier::guiControlData);
	auxAttribute.setUintAttribute(2147483648);
	setParamAuxAttribute(controlID::tempoAdjust_manual, auxAttribute);
...
bool PluginCore::reset(ResetInfo& resetInfo)
{
	...
	soundTouch.setSampleRate(resetInfo.sampleRate);
...
{
	soundTouch.setRate(rateAdjust_manual);
	soundTouch.setTempo(tempoAdjust_manual);
...
bool PluginCore::processAudioFrame(ProcessFrameInfo& processFrameInfo)
{
	...
	const soundtouch::SAMPLETYPE *samples = &processFrameInfo.audioInputFrame[0];

	soundTouch.putSamples(samples, 1);
	soundTouch.receiveSamples(&processFrameInfo.audioOutputFrame[0], 1);
	return true;
...
bool PluginCore::initPluginPresets()
{
	...
	setPresetParameter(preset->presetParameters, controlID::rateAdjust_manual, 1.000000);
	setPresetParameter(preset->presetParameters, controlID::tempoAdjust_manual, 1.000000);
	addPreset(preset);
